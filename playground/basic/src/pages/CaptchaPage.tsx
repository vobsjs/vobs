...
export function CaptchaPage() {
  const i18n = useI18n()
  const challenge = state<SliderCaptchaChallenge | null>(createDemoChallenge(), 'captcha.challenge')
  const status = state<'idle' | 'loading' | 'ready' | 'verifying' | 'verified' | 'expired' | 'error'>('ready', 'captcha.status')
  const error = state<unknown>(null, 'captcha.error')
  const closed = state(false, 'captcha.closed')
  let operation = 0
