...
export function ErrorDiagnosticsPage() {
  const renderFailure = state(false, 'devtools.errorLab.render')
  const effectFailure = state(false, 'devtools.errorLab.effect')
  const requestMessage = state('No request error has been triggered.', 'devtools.errorLab.request')
  const lastAction = state('No probe has been triggered.', 'devtools.errorLab.lastAction')
  ...
  const triggerGlobalError = (): void => {
    ...
    const filename = 'src/pages/ErrorDiagnosticsPage.tsx'
  ...
  const triggerUsageError = (): void => {
    ...
    devtools.reportError(
      ...
      }),
      {
        ...
        handled: true,
        recovery: 'handled',
        source: 'src/pages/ErrorDiagnosticsPage.tsx:91:7',
