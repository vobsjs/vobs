...
export function SSRPage() {
  const i18n = useI18n()
  const serverHtml = state('', 'playground.ssr.html')
  const snapshot = state('', 'playground.ssr.snapshot')
  const hydrationStatus = state<'idle' | 'success' | 'error'>('idle', 'playground.ssr.hydration')
  const mismatchMessage = state('', 'playground.ssr.mismatch')
