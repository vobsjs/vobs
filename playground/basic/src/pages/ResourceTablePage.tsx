...
export function ResourceTablePage() {
  ...
  const currentPage = state(1, 'resource.users.page')
  const currentPageSize = state(3, 'resource.users.pageSize')
  ...
  return (
    <>
      ...
      tableProps={{
        rowKey: user => user.id,
        get page() { return currentPage.value },
        get pageSize() { return currentPageSize.value },
        pageSizeOptions: [3, 10, 25, 50, 100],
        paginationMode: 'all',
        sortingMode: 'client',
        icons: {
          ascending: () => <Icon name="chevron-up" />,
          descending: () => <Icon name="chevron-down" />,
          unsorted: () => <Icon name="sort" />,
        ...
        onQueryChange: query => {
          ...
          currentPageSize.value = query.pageSize
        }
      }}
      />
      <ResourceLab />
...
function ResourceLab() {
  ...
  const writeStatus = state(i18n.t('resource.noWrite'), 'resource.writeStatus')
  const scenarioMessage = state(i18n.t('resource.scenarioReady'), 'resource.scenario.message')
  ...
  const serverTable = createComponent(KitDataTable<PlaygroundUser>, {
    ...
    paginationMode: 'all',
    get page() { return serverPage.value },
    get pageSize() { return serverPageSize.value },
    pageSizeOptions: [2, 5],
    onQueryChange: query => {
      serverPage.value = query.page
      serverPageSize.value = query.pageSize
    }
  })
