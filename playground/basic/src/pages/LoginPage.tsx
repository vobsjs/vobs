...
import { callProtectedApi, fetchProtectedWithReplay, getTokenInfo, jwtAuth } from '../jwt-auth'
...
export function LoginPage() {
  ...
  const mode = state<'session' | 'jwt'>('session', 'auth.login.mode')
...
function JwtConsole() {
  ...
  const i18n = useI18n()
  const nowSeconds = state(Math.floor(Date.now() / 1000), 'auth.jwt.now')
  const apiResult = state('', 'auth.jwt.apiResult')
  const apiError = state('', 'auth.jwt.apiError')
  const calling = state(false, 'auth.jwt.calling')
