...
import { httpScenario, playgroundHttp, uploadScenario } from '../../plugins/http'
import { syncScenario } from '../../plugins/sync'
...
export function AsyncPage() {
  ...
  const uploadMessage = state(i18n.t('async.chooseFile'))
  const syncMessage = state(i18n.t('async.noSync'))
  const httpState = state<'idle' | 'loading' | 'success' | 'error'>('idle')
  const httpMessage = state(i18n.t('async.httpDescription'))
